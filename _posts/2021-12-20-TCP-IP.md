...
먼저 TCP/IP 모델부터 알아보고, 이후에 URL 입력에 대해 어떤 일이 발생하는지 알아보자.

<br>

# TCP/IP 4계층 모형
**인터넷 프로토콜 스위트(Internet Protocol Suite)**는
**인터넷**에서 컴퓨터들이 서로 정보를 주고 받는데 쓰이는 **통신규약(프로토콜)의 모음**이다.
...
- DHCP, DNS, FTP, HTTP, HTTPS, IMAP, LDAP, POP, SMTP, SSH, TELNET, TLS/SSL, ...

## 2. Transport Layer

- 송신된 데이터를 수신 측 애플리케이션에 확실하게 전달하게 함.
- Port 번호를 사용해서 애플리케이션을 찾아 주는 역할.
- TCP, QUIC, UDP, ...

## 3. Internet Layer

- 수신 측까지 데이터를 전달하기 위해 사용.
- IP(IPv4, IPv6), ARP, ICMP, ICMPv6, IPsec, ...

## 4. Network Access Layer

- 네트워크에 직접 연결된 기기 간 전송을 할 수 있게 함.
- 물리적 주소인 MAC 주소를 사용함.
...
- Internet Layer의 IP주소와 Network Access Layer의 MAC주소를 통해 데이터가 올바른 목적지에 도착했으니 2 계층에 대한 설명은 생략한다.
...
HTTP의 요청과 응답 과정이 끝나면 연결을 종료해야 한다. 이 작업을 **4-Way-Handshaking** 이라고 부른다.
